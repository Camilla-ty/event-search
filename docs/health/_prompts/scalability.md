...
7. When (and only when) publication is explicitly requested, the ONLY files you may
   ...
   - the new immutable report: docs/health/scalability/{{CYCLE}}-scalability.md
...
Create docs/health/scalability/{{CYCLE}}-scalability.md from
...
- Findings: FULL write-up for NEW SCALE Findings (all fields from STEP 4); existing
  SCALE Findings by ID + delta only — never restate full bodies; never rewrite old
  reports.
...
| Document | Role |
|---|---|
| [`../README.md`](../README.md) | Framework v1.1 operating rules (Quarterly Scalability) |
