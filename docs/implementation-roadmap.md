# EventPixels — Implementation Roadmap (v1)
...
## Phase 2 — Sponsor import migration
...
- [ ] Migration applies cleanly on target environment
- [ ] All four import tables queryable via service role
- [ ] Anon/authenticated cannot SELECT import tables
- [ ] Duplicate live sponsor INSERT fails (constraint smoke test)
- [ ] Second active batch same edition fails (partial unique smoke test)
- [ ] No regression on existing `event_sponsors` reads (14 rows intact)
- [ ] Migration recorded in `supabase/migrations/`
...
- [ ] Integration tests or manual script: happy path upload → publish
- [ ] Domain auto-accept sets `auto_ready`; name-only sets `needs_review`
- [ ] Additive publish: existing live sponsors not in file remain
- [ ] Discard: draft links gone; companies remain
- [ ] Duplicate in-file blocks import-to-draft until resolved
- [ ] `event_sponsors` uniqueness enforced on publish
- [ ] Action log entries on upload, bulk accept, import-to-draft, publish, discard
- [ ] All routes reject non-admin
...
- [ ] End-to-end UI: upload Excel → publish → live sponsors visible on edition tab
- [ ] **Create & import sponsors** path works without edition re-select
- [ ] Resume from history and dashboard
- [ ] Bulk accept exact domain matches
- [ ] Duplicate-in-file resolution blocks progress until done
- [ ] Additive publish copy on checklist
- [ ] Discard flow with company retention message
- [ ] One active draft per edition enforced in UI
- [ ] Save & exit preserves batch state
- [ ] `npm run build` passes
