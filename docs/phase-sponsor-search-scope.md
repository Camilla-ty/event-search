# Phase — Sponsor Search: Implementation Scope

**Status:** Approved — S1 server + S2 UI + **v2 grouped results** (S3 rate-limit release gate pending)
**Version:** v1.2
**Last updated:** 2026-07-25

Short implementation scope for **Sponsor Search** on the public event edition Sponsors tab. Based on the approved architecture and security audit (2026-07-24); **authenticated-only** as of v1.1; **tier-grouped search results** as of v1.2 / Sponsor Search v2.
...
| Result layout (v2) | Client groups the flat capped `items` with `groupSponsorsByTier`; render **always-expanded** tier panels (shared `PublicSponsorTierPanel` chrome) — same header/borders/rows as the live roster; **no** accordion toggle, lock, lazy load, or Load more |
| Header counts | Panel counts are **match counts within the ≤20 response**, not full edition tier totals from `tierSummaries` |
...
| **S2.1 — Grouped results (v2)** | ✅ Client-side `groupSponsorsByTier`; shared `PublicSponsorTierPanel`; always-expanded match-count panels; search-prefixed a11y ids; API/RPC unchanged |
...
19. Search results are grouped by sponsorship tier (existing tier order); all matching tiers are shown expanded with exact stored labels.
20. Tier panel counts reflect matches in the capped response, not full roster tier totals.
...
27. Grouped search UI does not change the flat API contract (still no `total` / `hasMore` / server-side group payload).
...
| Tier grouping helper | `src/features/events/lib/groupSponsorsByTier.ts` |
| Shared tier panel chrome | `src/features/events/components/detail/PublicSponsorTierPanel.tsx` |
...
| 2026-07-25 | **v2 grouped results:** client `groupSponsorsByTier` + shared `PublicSponsorTierPanel`; always-expanded match panels; API/RPC unchanged |
