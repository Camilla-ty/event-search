...
> Last updated: 2026-08-01 (ADR-004 Participated Events Series-hub UX prototype; SB4 ops)
...
| Entity | Table | Notes |
|---|---|---|
| **Event Series** | `event_series` | Recurring event brand (name, slug, website, logo, lifecycle). Optional nullable **`company_profile_id`** → `companies.id` (ADR-004 same-brand link; FK `ON DELETE RESTRICT`; unique so one Company links to at most one Series). **Admin:** Series detail can search/link/replace/unlink; Company detail shows read-only reverse Series. **Public prototype (2026-08):** Series hub may show a **Participated Events** tab (sponsor appearances of the linked Company) when results exist; Company `/sponsors/...` profiles remain visible (no redirect/hide). Reciprocal Series↔Company header links are hidden while this UX is evaluated. Migration: `20260731120000_event_series_company_profile_id.sql`. |
...
| **Companies** | `companies` | Canonical company entity. No separate sponsors table — "sponsor" = company linked to an edition. Optional `restricted_at` excludes public discovery/profile. Optional undirected **Related Companies** links in `company_related_companies` (admin + public sponsor profile discovery rail at page bottom when links exist; not merge / not same-brand). |
...
| **Keywords** | `keyword`, `event_series_keyword` | Series tags via M2M. Current catalog intent: top-level Event Series keywords are the four industries `AI`, `FinTech`, `HealthTech`, `Crypto & Blockchain` (one assignment per series after `20260825130000_event_series_keywords_four_only.sql`). Editions inherit series keywords (read-only chips). Older keyword catalog rows (e.g. `bitcoin`) may still exist for cleanup later; research pages were wiped (`20260825140000_delete_all_draft_research_pages.sql`) and will be rebuilt under the four-keyword taxonomy. |
| **Topic × Location Research Pages** | `topic_region_research_pages` | Admin-managed approval table (table name predates Country support). `status` = `draft` \| `published`. Optional `year` (`NULL` = all years). Location is **exactly one** of `region_id` or `country_id` (`CHECK num_nonnulls(region_id, country_id) = 1`), with a partial unique index per location kind on `(topic_keyword_id, location, COALESCE(year, 0))`. Published pages: `/events/topics/{topicSlug}/regions/{regionSlug}` or `/events/topics/{topicSlug}/countries/{countrySlug}`, each with optional `/years/{year}`. Content dynamically computed with DB year filter when scoped. Published rows are exposed via `topic_region_research_pages_published` (`location_type`, `location_name`, `location_slug`). Migrations: `20260729120000_topic_region_research_pages.sql`, `20260730120000_research_pages_optional_year.sql`, `20260826010000_research_pages_country_support.sql`. |
...
**Admin — exhibitors (E1–E6)** — Edition **Exhibitors** tab: add / edit tier / remove / within-tier reorder (drag-and-drop plus Move up / down; Save order persists `display_order`); `assertCompanyLinkable` on create; company-merge exhibitor conflict resolution. **Manual-only** `last_reviewed_at` (no auto-touch on exhibitor CRUD/reorder/import publish). **Bulk Upload (E6):** edition-scoped independent pipeline (`src/features/exhibitor-import/**`, `/admin/exhibitor-imports`, migration `20260726120000_exhibitor_import_v1.sql`) — spreadsheet Exhibitor Tier | Exhibitor Label | Name | Website; publish writes **`event_exhibitors` only** (never `event_sponsors`). Entry from edition Exhibitors tab (no global exhibitor-imports hub). See [exhibitor-design.md](./exhibitor-design.md).

**Admin — edition research metadata** — `last_reviewed_at` auto-updates on meaningful profile saves, live sponsor add/remove/tier edits, organizer add/remove/role-label edits, and qualifying **sponsor** import publish. Edition create keeps `last_reviewed_at` NULL. Manual-only Last reviewed saves are preserved. The edition **Primary source URL** field was removed (column `event_editions.primary_source_url` dropped 2026-08-26); `last_reviewed_at` is the only edition research-metadata field. Sponsor/organizer reorder/move and draft import steps do not touch. **Exhibitor** mutations and exhibitor-import publish do **not** auto-touch. Company merge auto-touches editions when organizer links are repointed. See [phase-edition-last-reviewed-automation-scope.md](./phase-edition-last-reviewed-automation-scope.md).
...
**Admin — Research Pages** — Topic × Location research page management at `/admin/research-pages`, where Location is a Region or a Country. List, create (draft only; Region/Country toggle; optional Year, blank = all years), preview (draft + published; loader uses `record.year`), publish, unpublish. Publish sets `status=published` and `published_at=now()`; unpublish reverts to draft and removes public access. Public routes: `/events/topics/[topicSlug]/regions/[regionSlug]` and `/events/topics/[topicSlug]/countries/[countrySlug]`, each with an optional `/years/[year]` variant. Region and Country are separate URL namespaces, so the same slug in each resolves to different pages. Quality gate and indexability are shared by both location kinds (`index` when the gate passes, `noindex` when it fails) — published pages render regardless of gate. Sitemap includes published + gate-passing pages. Published pages for a topic are also linked from `/topics/[slug]` via the “By location” block. Content is currently one draft (`Crypto & Blockchain` × `Singapore` × 2026) created to verify Country support; the earlier drafts were deleted in `20260825140000_delete_all_draft_research_pages.sql`, and nothing is published. Migrations: `20260729120000_topic_region_research_pages.sql`, `20260730120000_research_pages_optional_year.sql`, `20260826010000_research_pages_country_support.sql`.
