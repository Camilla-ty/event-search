...
| **Topic × Region Research Pages** | `topic_region_research_pages` | Admin-managed approval table. `status` = `draft` \| `published`. Optional `year` (`NULL` = all years). Only published all-years rows produce public pages at `/events/topics/{topicSlug}/regions/{regionSlug}` today; year-scoped public routes deferred. Content dynamically computed. Migrations: `20260729120000_topic_region_research_pages.sql`, `20260730120000_research_pages_optional_year.sql`. |
...
**Admin — Research Pages** — Topic × Region research page management at `/admin/research-pages`. List, create (draft only; optional Year, blank = all years), preview (draft + published; preview data still all-years until Phase B), publish, unpublish. Publish sets `status=published` and `published_at=now()`; unpublish reverts to draft and removes public access. Generic public route at `/events/topics/[topicSlug]/regions/[regionSlug]` renders only published **all-years** pages (`year IS NULL`). Quality gate controls indexability (`index` when passes, `noindex` when fails) — published pages render regardless of gate. Sitemap includes only published + gate-passing all-years pages. Year-scoped public routes deferred. Bitcoin × Asia seeded as published with `year = NULL`. Migrations: `20260729120000_topic_region_research_pages.sql`, `20260730120000_research_pages_optional_year.sql`.
