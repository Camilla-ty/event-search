...
| **Topic × Region Research Pages** | `topic_region_research_pages` | Admin-managed approval table. `status` = `draft` \| `published`. Optional `year` (`NULL` = all years). Published all-years pages: `/events/topics/{topicSlug}/regions/{regionSlug}`. Published year-scoped pages: `/events/topics/{topicSlug}/regions/{regionSlug}/years/{year}`. Content dynamically computed with DB year filter when scoped. Migrations: `20260729120000_topic_region_research_pages.sql`, `20260730120000_research_pages_optional_year.sql`. |
...
**Admin — Research Pages** — Topic × Region research page management at `/admin/research-pages`. List, create (draft only; optional Year, blank = all years), preview (draft + published; loader uses `record.year`), publish, unpublish. Publish sets `status=published` and `published_at=now()`; unpublish reverts to draft and removes public access. Public routes: all-years at `/events/topics/[topicSlug]/regions/[regionSlug]`; year-scoped at `/events/topics/[topicSlug]/regions/[regionSlug]/years/[year]`. Quality gate controls indexability (`index` when passes, `noindex` when fails) — published pages render regardless of gate. Sitemap includes published + gate-passing pages (all-years and year-scoped). Bitcoin × Asia seeded as published with `year = NULL`. Migrations: `20260729120000_topic_region_research_pages.sql`, `20260730120000_research_pages_optional_year.sql`.
