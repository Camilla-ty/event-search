...
import { reorderEventOrganizerLinkAdmin } from "@/src/features/organizers/server/eventOrganizerAdmin";
import { requireAdminApi } from "@/src/lib/auth/requireAdminApi";
import { validateEventOrganizerReorderBody } from "@/src/lib/validation/eventOrganizer";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const validated = validateEventOrganizerReorderBody(body);
  if (!validated.ok) {
  ...
  try {
    ...
    const status =
      message === "Organizer link not found." ? 404 : 500;
