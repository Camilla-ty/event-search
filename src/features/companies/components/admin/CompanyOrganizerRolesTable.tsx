...
import type { CompanyOrganizerRoleRow } from "@/src/features/organizers/server/companyOrganizerAdmin";

type CompanyOrganizerRolesTableProps = {
  organizerRoles: CompanyOrganizerRoleRow[];
};

export function CompanyOrganizerRolesTable({
  organizerRoles,
}: CompanyOrganizerRolesTableProps) {
  if (organizerRoles.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-sm text-slate-600">
        This company is not an organizer of any event edition yet.
      </p>
    );
  }
