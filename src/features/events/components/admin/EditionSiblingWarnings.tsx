...
export function EditionSiblingWarnings({
  ...
  return (
    ...
    >
      <p className="font-medium">Related event editions for this event series in {year}</p>
      <ul className="mt-2 list-disc space-y-2 pl-5">
        {cityMatches.length > 0 ? (
          <li>
            An event edition already exists for this event series, year, and city
            {cityLabel !== "" ? ` (${cityLabel})` : ""}:
        ...
        <li>
          {siblings.length === 1
            ? "One other event edition exists"
            : `${siblings.length} other event editions exist`}{" "}
          for this event series in {year}. Use a distinct name and slug
          {cityId === "" ? " — add a city or include location in the event edition name" : ""}:
          <ul className="mt-1 list-none space-y-1 pl-0">
