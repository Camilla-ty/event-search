    ...
    <div className="space-y-2">
      <label className="block space-y-1">
        <span className="text-sm font-medium text-slate-700">
          Merged into event series <span className="text-red-600">*</span>
        </span>
        ...
        <input
          ...
          placeholder={`Search event series (min ${SEARCH_MIN_CHARS} characters)…`}
          value={search}
      ...
      {!selected && term.length >= SEARCH_MIN_CHARS ? (
        <div className="rounded-lg border border-slate-200 bg-white">
          {loading ? (
            ...
            <p className="px-3 py-2 text-sm text-slate-500">No event series found.</p>
          ) : (
