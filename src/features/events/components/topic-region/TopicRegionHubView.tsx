...
type TopicRegionHubViewProps = {
  data: TopicRegionHubPageData;
};

export function TopicRegionHubView({ data }: TopicRegionHubViewProps) {
  return (
    <section className="mx-auto max-w-6xl space-y-8">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          {data.summary ? (
            ...
            <p className="text-sm text-slate-500">
              No summary available yet for this combination.
            </p>
          )}
      ...
      <section aria-labelledby="topic-region-events-heading" className="space-y-4">
        <h2
          id="topic-region-events-heading"
        ...
        <p>
          <Link href={data.topicHubPath} className={`text-sm ${brandLinkClass}`}>
            View all {data.topicName} events →
          </Link>
        </p>
      </section>
      ...
      <section
        id="sponsors"
        aria-labelledby="topic-region-sponsors-heading"
        className="space-y-4"
      >
        <div className="space-y-1">
          <h2
            id="topic-region-sponsors-heading"
            className="text-lg font-semibold text-slate-900"
          >
            Sponsors on these {data.topicName} events
          </h2>
          <p className="text-sm font-medium text-slate-700">
            {data.totalSponsorCount} companies recorded as sponsors of the {data.topicName}
            {" "}events listed above.
          </p>
        </div>
