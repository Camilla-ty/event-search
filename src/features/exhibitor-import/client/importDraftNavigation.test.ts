...
} from "@/src/features/exhibitor-import/client/importDraftNavigation";
import { resolveStepForBatch } from "@/src/features/exhibitor-import/client/resumeStep";
...
describe("importDraftNavigation", () => {
  it("builds the draft wizard href", () => {
    assert.equal(
      ...
      `/admin/exhibitor-imports/${BATCH_ID}?step=draft`,
    );
  });
...
describe("review to draft client navigation policy", () => {
  ...
  it("ExhibitorImportFlow navigates with router.replace and hides review for draft batches", () => {
    const source = readFileSync(
      path.join(process.cwd(), "src/features/exhibitor-import/components/ExhibitorImportFlow.tsx"),
      "utf8",
    );
...
describe("materialize endpoints after draft completion", () => {
  it("returns completed company materialization instead of 409 when batch is draft", () => {
    const source = readFileSync(
      path.join(process.cwd(), "src/features/exhibitor-import/server/exhibitorImportAdmin.ts"),
      "utf8",
    );
    assert.match(
      source,
...
describe("duplicate safety during materialization", () => {
  it("skips rows that already have resolved_company_id", () => {
    const source = readFileSync(
      path.join(process.cwd(), "src/features/exhibitor-import/server/materializeCompanies.ts"),
      "utf8",
    );
  ...
  it("materializes draft links only for rows missing draft_link_id", () => {
    const source = readFileSync(
      path.join(process.cwd(), "src/features/exhibitor-import/server/materializeDraft.ts"),
      "utf8",
    );
