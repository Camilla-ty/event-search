...
import { ImportProgressMessage } from "@/src/features/exhibitor-import/components/ImportProgressMessage";

describe("ImportProgressMessage", () => {
  it("preserves status markup and spinner classes", () => {
    const html = renderToStaticMarkup(
      React.createElement(ImportProgressMessage, { message: "Importing exhibitors…" }),
    );
    ...
    assert.match(html, /Importing exhibitors…/);
  });
});
