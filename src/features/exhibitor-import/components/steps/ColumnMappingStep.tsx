...
export function ColumnMappingStep({ spreadsheetHeaders }: ColumnMappingStepProps) {
  ...
  const { batch, goToStep, updateBatch } = useExhibitorImportWizard();
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          Map spreadsheet columns to exhibitor tier, exhibitor label, company name, and website.
        </p>
      </div>
      ...
      <div className="grid max-w-lg gap-4">
        {renderSelect("Exhibitor tier column", "tier_rank")}
        {renderSelect("Exhibitor label column", "tier_label")}
      ...
      <div className="flex gap-2">
        <Button
          variant="secondary"
          onClick={() =>
            router.push(`/admin/events/editions/${batch.event_edition_id}?tab=exhibitors`)
          }
        >
          Save & exit
        </Button>
