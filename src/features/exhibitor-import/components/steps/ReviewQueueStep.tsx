...
import type { RowSummary, ExhibitorImportRow } from "../../client/types";
import { EXHIBITOR_IMPORT_MAX_ROWS } from "../../types";
...
function importStatusMessage(summary: RowSummary, canImportToDraft: boolean): string {
  if (canImportToDraft) {
    return "All rows resolved. Ready to create the draft event edition.";
  }
...
function duplicateClusterSize(row: ExhibitorImportRow): number | null {
  const size = row.duplicate_cluster_size ?? null;
  return typeof size === "number" && size > 1 ? size : null;
}

function duplicateHelperText(row: ExhibitorImportRow): string | null {
...
function sponsorshipLabel(row: ExhibitorImportRow): { primary: string; secondary: string | null } {
...
function duplicateStatusLabel(row: ExhibitorImportRow): { primary: string; secondary: string | null } | null {
...
export function ReviewQueueStep({ initialSummary }: ReviewQueueStepProps) {
  const { batch, goToStep, openDraftStep } = useExhibitorImportWizard();
  ...
  const [rows, setRows] = useState<ExhibitorImportRow[]>([]);
  const [filter, setFilter] = useState<FilterKey>("needs_review");
  const [selectedRow, setSelectedRow] = useState<ExhibitorImportRow | null>(null);
  ...
  const loadRowsForFilter = useCallback(
    async (activeFilter: FilterKey) => {
      const listed = await fetchRows(batch.id, {
        ...
        page: 1,
        pageSize: EXHIBITOR_IMPORT_MAX_ROWS,
      });
      if (!listed.ok) {
  ...
  async function handleKeepDuplicateRow(row: ExhibitorImportRow) {
                ...
                <table className="min-w-full text-left text-sm">
                  <thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase text-slate-500">
                    <tr>
                      ...
                      <th className="px-4 py-2">Exhibitor link</th>
