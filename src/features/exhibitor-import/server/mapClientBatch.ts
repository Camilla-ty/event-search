...
export function mapBatchForClient(raw: Record<string, unknown>): ExhibitorImportBatch {
  return {
    id: String(raw.id),
    event_edition_id: String(raw.event_edition_id),
    status: raw.status as ExhibitorImportBatch["status"],
    processing_phase:
      raw.processing_phase === null || typeof raw.processing_phase === "string"
        ? (raw.processing_phase as ExhibitorImportBatch["processing_phase"])
        : null,
