...
import { isUniqueViolation, ExhibitorImportHttpError } from "./errors";
...
export async function resolveCreateNewCompanyId(
  ...
  if (!name) {
    throw new ExhibitorImportHttpError(422, `Row ${row.excel_row_number} missing company name.`);
  }
...
export async function resolveCompanyIdForRow(
  ...
  if (row.decision_type === "use_matched" || row.decision_type === "choose_different") {
    ...
    if (!id) {
      throw new ExhibitorImportHttpError(422, `Row ${row.excel_row_number} missing matched company.`);
    }
  ...
  throw new ExhibitorImportHttpError(422, `Row ${row.excel_row_number} has no company resolution.`);
}
