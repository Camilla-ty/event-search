...
async function insertDraftLink(
  ...
  try {
    ...
    throw new ExhibitorImportHttpError(
      422,
      error instanceof Error ? error.message : "Company cannot be linked as an exhibitor.",
...
export async function materializeDraftLinks(
  ...
  for (const row of rows) {
    ...
    const tier = row.mapped_tier_rank;
    if (!companyId || tier === null) {
      throw new ExhibitorImportHttpError(422, `Row ${row.excel_row_number} missing company or tier.`);
    }
