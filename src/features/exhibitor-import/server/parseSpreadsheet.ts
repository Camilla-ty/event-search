...
export function parseWithColumnMapping(
  ...
  if (dataMatrix.length > EXHIBITOR_IMPORT_MAX_ROWS) {
    throw new Error(
      `File has ${dataMatrix.length} data rows; maximum is ${EXHIBITOR_IMPORT_MAX_ROWS}.`,
    );
  }
...
/** Match tier rank / exhibitor tier headers (spaces, underscores, hyphens). */
...
/** Match tier label / exhibitor label headers (spaces, underscores, hyphens). */
const TIER_LABEL_HEADER_PATTERNS = [
  /^exhibitor[\s_-]+label$/i,
  /^exhibitor[\s_-]*label$/i,
  /^tier_label$/i,
  /^tier[\s_-]+label$/i,
  /^sponsor[\s_-]+label$/i,
