...
export function resolveRowCompanyName(row: SponsorImportRow): string {
  return (row.normalized_company_name ?? row.raw_company_name ?? "").trim();
}

export function resolveRowDomain(row: SponsorImportRow): string {
...
export function hasBlockingValidation(row: SponsorImportRow): boolean {
  return row.has_blocking_validation === true;
}

export function getBulkCreateNewBlockReason(row: SponsorImportRow): BulkCreateNewBlockReason | null {
...
export function getBulkCreateNewButtonState(
  ...
  const selectedRows: SponsorImportRow[] = [];
  let notLoaded = 0;
