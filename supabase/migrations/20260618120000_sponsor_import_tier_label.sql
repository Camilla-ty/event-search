-- Carry sponsor tier labels from spreadsheet through import rows and draft links.
-- Publish RPC: spreadsheet is source of truth for tier_label (and existing tier_rank).

ALTER TABLE public.sponsor_import_rows
  ADD COLUMN IF NOT EXISTS raw_tier_label text,
  ADD COLUMN IF NOT EXISTS mapped_tier_label text;

ALTER TABLE public.sponsor_import_draft_links
