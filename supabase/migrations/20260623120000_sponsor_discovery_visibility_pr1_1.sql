-- PR1.1 — Split sponsor discovery visibility model
--
-- Global /sponsors (p_event_slug IS NULL):
--   companies-first + company_sponsor_stats (all tiers, aggregate only).
--   Anon and authenticated see the same eligible company set and counts.
--
-- Event-filter /sponsors?event= (p_event_slug IS NOT NULL):
--   event_sponsors + SECURITY INVOKER (RLS applies; anon tier-1 only).
--
-- company_sponsor_stats exposes eligibility aggregates only — no tier rows.

CREATE OR REPLACE VIEW public.company_sponsor_stats
WITH (security_invoker = false)
AS
SELECT
  es.company_id,
  count(DISTINCT es.event_editions_id)::integer AS sponsored_edition_count,
  max(ee.start_date) AS latest_activity_at
FROM public.event_sponsors es
INNER JOIN public.event_editions ee ON ee.id = es.event_editions_id
GROUP BY es.company_id;

COMMENT ON VIEW public.company_sponsor_stats IS
  'All-tier sponsor aggregates per company (edition count + latest start_date). '
  'security_invoker=false so anon/authenticated can resolve global discovery eligibility '
  'without reading tier-2+ event_sponsors rows. Exposes no tier or per-edition detail.';

GRANT SELECT ON public.company_sponsor_stats TO anon, authenticated;
...
COMMENT ON FUNCTION public.sponsor_discovery_page(text, text, text, integer, integer) IS
  'Sponsor discovery: global mode (companies + company_sponsor_stats, all-tier eligibility) '
  'when p_event_slug IS NULL; event-filter mode (event_sponsors + RLS) when p_event_slug IS set. '
  'SECURITY INVOKER. JSON schema documented in migration 20260622120000.';
