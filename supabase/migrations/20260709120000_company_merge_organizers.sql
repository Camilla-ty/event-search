-- Company merge: repoint event_edition_organizers + conflict resolution (Organizer O2)

CREATE OR REPLACE FUNCTION public._company_merge_organizer_strategy(
  p_resolutions jsonb,
...
BEGIN
  FOR v_item IN
    SELECT value
    FROM jsonb_array_elements(COALESCE(p_resolutions -> 'organizer_conflicts', '[]'::jsonb))
  LOOP
    IF (v_item ->> 'event_edition_id')::uuid = p_event_edition_id THEN
      v_strategy := v_item ->> 'strategy';
      IF v_strategy IN ('keep_canonical', 'keep_duplicate_role') THEN
        RETURN v_strategy;
      END IF;
...
REVOKE ALL ON FUNCTION public._company_merge_organizer_strategy(jsonb, uuid) FROM PUBLIC;
...
CREATE OR REPLACE FUNCTION public._company_merge_process_organizers(
  p_canonical_company_id uuid,
  p_duplicate_company_id uuid,
  p_preview jsonb,
  p_resolutions jsonb
...
DECLARE
  ...
  v_edition_id uuid;
  v_strategy text;
  v_dup_organizer public.event_edition_organizers%ROWTYPE;
  v_can_organizer public.event_edition_organizers%ROWTYPE;
...
BEGIN
  FOR v_conflict IN
    SELECT value
    FROM jsonb_array_elements(COALESCE(p_preview -> 'organizer_conflicts', '[]'::jsonb))
  LOOP
    v_edition_id := (v_conflict ->> 'event_edition_id')::uuid;
    v_strategy := public._company_merge_organizer_strategy(p_resolutions, v_edition_id);

    SELECT * INTO v_dup_organizer
    FROM public.event_edition_organizers
    WHERE id = (v_conflict -> 'duplicate_link' ->> 'event_edition_organizer_id')::uuid;

    SELECT * INTO v_can_organizer
    FROM public.event_edition_organizers
    WHERE id = (v_conflict -> 'canonical_link' ->> 'event_edition_organizer_id')::uuid;

    IF v_strategy = 'keep_canonical' THEN
      v_deleted_log := v_deleted_log || jsonb_build_array(
        jsonb_build_object(
          'id', v_dup_organizer.id,
          'row', jsonb_build_object(
            'company_id', v_dup_organizer.company_id,
            'event_editions_id', v_dup_organizer.event_editions_id,
            'role_label', v_dup_organizer.role_label,
            'display_order', v_dup_organizer.display_order
          ),
          'reason', 'organizer_conflict_keep_canonical',
          'event_edition_id', v_edition_id
        )
      );
      DELETE FROM public.event_edition_organizers WHERE id = v_dup_organizer.id;
      v_deleted := v_deleted + 1;
      v_affected_editions := array_append(v_affected_editions, v_edition_id);
    ELSIF v_strategy = 'keep_duplicate_role' THEN
      v_updated_log := v_updated_log || jsonb_build_array(
        jsonb_build_object(
          'id', v_can_organizer.id,
          'before', jsonb_build_object(
            'role_label', v_can_organizer.role_label,
            'display_order', v_can_organizer.display_order
          ),
          'after', jsonb_build_object(
            'role_label', v_dup_organizer.role_label,
            'display_order', v_dup_organizer.display_order
          ),
          'reason', 'organizer_conflict_keep_duplicate_role'
        )
      );
      UPDATE public.event_edition_organizers
      SET
        role_label = v_dup_organizer.role_label,
        display_order = v_dup_organizer.display_order,
        updated_at = v_now
      WHERE id = v_can_organizer.id;
  ...
  WITH repointed AS (
    UPDATE public.event_edition_organizers eeo
    SET
      company_id = p_canonical_company_id,
      updated_at = v_now
    WHERE eeo.company_id = p_duplicate_company_id
    RETURNING eeo.id, eeo.event_editions_id, eeo.role_label, eeo.display_order
  )
  SELECT
    COUNT(*)::integer,
    COALESCE(
...
REVOKE ALL ON FUNCTION public._company_merge_process_organizers(uuid, uuid, jsonb, jsonb) FROM PUBLIC;
