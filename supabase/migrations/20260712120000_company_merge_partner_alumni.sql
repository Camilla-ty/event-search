-- Company merge: repoint Partner Alumni version members (PA5)
-- Prerequisite: 20260711120000_partner_alumni_v2_versions.sql (table may be absent on pre-PA1' DBs)
...
CREATE OR REPLACE FUNCTION public._company_merge_process_partner_alumni(
  p_canonical_company_id uuid,
  p_duplicate_company_id uuid,
  p_preview jsonb
)
...
DECLARE
  v_conflict jsonb;
  v_version_id uuid;
  v_dup_member public.event_partner_alumni_version_companies%ROWTYPE;
  v_can_member public.event_partner_alumni_version_companies%ROWTYPE;
  v_merged_order integer;
...
BEGIN
  IF to_regclass('public.event_partner_alumni_version_companies') IS NULL THEN
    RETURN jsonb_build_object(
      'repointed', 0,
      'deleted', 0,
      'updated', 0,
  ...
  FOR v_conflict IN
    ...
    FROM jsonb_array_elements(
      COALESCE(p_preview -> 'partner_alumni_version_conflicts', '[]'::jsonb)
    )
  LOOP
    v_version_id := (v_conflict ->> 'event_partner_alumni_version_id')::uuid;

    SELECT * INTO v_dup_member
    FROM public.event_partner_alumni_version_companies
    WHERE id = (v_conflict -> 'duplicate_link' ->> 'version_member_id')::uuid;

    SELECT * INTO v_can_member
    FROM public.event_partner_alumni_version_companies
    WHERE id = (v_conflict -> 'canonical_link' ->> 'version_member_id')::uuid;

    v_merged_order := LEAST(v_can_member.display_order, v_dup_member.display_order);

    IF v_merged_order IS DISTINCT FROM v_can_member.display_order THEN
      v_updated_log := v_updated_log || jsonb_build_array(
        jsonb_build_object(
          'id', v_can_member.id,
          'event_partner_alumni_version_id', v_version_id,
          'before', jsonb_build_object('display_order', v_can_member.display_order),
          'after', jsonb_build_object('display_order', v_merged_order),
          'reason', 'partner_alumni_conflict_preserve_lower_display_order'
        )
      );
      UPDATE public.event_partner_alumni_version_companies
      SET
        display_order = v_merged_order,
        updated_at = v_now
      WHERE id = v_can_member.id;
    ...
    v_deleted_log := v_deleted_log || jsonb_build_array(
      jsonb_build_object(
        ...
        'row', jsonb_build_object(
          'company_id', v_dup_member.company_id,
          'event_partner_alumni_version_id', v_dup_member.event_partner_alumni_version_id,
          'display_order', v_dup_member.display_order
        ),
        'reason', 'partner_alumni_conflict_dedupe_duplicate',
        'event_partner_alumni_version_id', v_version_id
      )
    );
    DELETE FROM public.event_partner_alumni_version_companies WHERE id = v_dup_member.id;
    v_deleted := v_deleted + 1;
  END LOOP;

  WITH repointed AS (
    UPDATE public.event_partner_alumni_version_companies pavc
    SET
    ...
    RETURNING
      pavc.id,
      pavc.event_partner_alumni_version_id,
      pavc.display_order
  )
  SELECT
    COUNT(*)::integer,
      ...
      jsonb_agg(
        jsonb_build_object(
          ...
          'before', jsonb_build_object(
            ...
            'event_partner_alumni_version_id', r.event_partner_alumni_version_id,
  ...
  INTO v_repointed, v_repointed_log
  FROM repointed r;
