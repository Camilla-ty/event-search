-- Company public restriction (V1): restricted_at timestamp + discovery exclusion.
-- Only active companies may be restricted; merged companies must remain public (restricted_at IS NULL).

ALTER TABLE public.companies
  ADD COLUMN restricted_at timestamptz;

ALTER TABLE public.companies
  ADD CONSTRAINT companies_restricted_active_only CHECK (
    restricted_at IS NULL
    OR status = 'active'::public.company_status
  );

CREATE INDEX companies_publicly_displayable_idx
  ON public.companies (id)
  WHERE restricted_at IS NULL
    AND status = 'active'::public.company_status;

COMMENT ON COLUMN public.companies.restricted_at IS
  'When set, the company is excluded from public discovery and profile pages; '
  'event sponsor lists show the company name with a content-policy message only.';
