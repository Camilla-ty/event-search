-- P4A follow-up: restore approved public domain/website in discovery RPC row JSON.
-- Keeps P4A exclusions (short_description, tier_rank, tier_label, event.id).
...
COMMENT ON FUNCTION public.sponsor_discovery_page(text, text, text, integer, integer) IS
  'Sponsor discovery: public row JSON includes domain/website; excludes short_description, '
  'tier_rank, tier_label, and event.id. SECURITY INVOKER.';
