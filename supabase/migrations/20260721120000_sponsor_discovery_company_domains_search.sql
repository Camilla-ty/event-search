-- Sponsor discovery: match verified alias domains in company_domains (primary + non-primary).
-- company_domains stays service-role-only; matching uses a scoped SECURITY DEFINER helper.

CREATE OR REPLACE FUNCTION public.__company_matches_verified_domain_search(
  p_company_id uuid,
  p_query text
)
RETURNS boolean
  ...
  SELECT
    ...
    AND EXISTS (
      ...
      WHERE cd.company_id = p_company_id
        AND cd.domain ILIKE '%' || trim(p_query) || '%'
    );
$$;

COMMENT ON FUNCTION public.__company_matches_verified_domain_search(uuid, text) IS
  'Internal discovery helper: true when p_company_id owns a company_domains row matching p_query. '
  'SECURITY DEFINER so sponsor_discovery_page (SECURITY INVOKER) can search alias domains without '
  'granting anon/authenticated SELECT on company_domains.';
