-- Exhibitors v1 (E1) — event_exhibitors join table + company merge support
-- Design: docs/exhibitor-design.md (Approved)

-- =============================================================================
-- Phase A — event_exhibitors table
-- =============================================================================
...
COMMENT ON TABLE public.event_exhibitors IS
  'Edition-scoped exhibitor links to companies. One row per (edition, company). Sponsor-like tier metadata; Name/Website live on companies.';

-- =============================================================================
-- Phase B — unique constraint + indexes
-- =============================================================================

CREATE UNIQUE INDEX event_exhibitors_edition_company_unique
  ON public.event_exhibitors (event_editions_id, company_id);

CREATE INDEX event_exhibitors_edition_tier_order_idx
  ON public.event_exhibitors (event_editions_id, tier_rank, display_order);

CREATE INDEX event_exhibitors_company_id_idx
  ON public.event_exhibitors (company_id);

-- =============================================================================
-- Phase C — RLS and grants (mirror organizers / catalog public read)
-- =============================================================================

ALTER TABLE public.event_exhibitors ENABLE ROW LEVEL SECURITY;

CREATE POLICY "event_exhibitors_select_anon_all"
  ON public.event_exhibitors
  FOR SELECT
  TO anon
  USING (true);

CREATE POLICY "event_exhibitors_select_authenticated_all"
  ON public.event_exhibitors
  FOR SELECT
  TO authenticated
  USING (true);

REVOKE ALL ON TABLE public.event_exhibitors FROM anon, authenticated;
GRANT SELECT ON TABLE public.event_exhibitors TO anon, authenticated;

-- =============================================================================
-- Phase D — company merge: Sponsor-style edition-level exhibitor conflicts
-- =============================================================================

DROP FUNCTION IF EXISTS public._company_merge_exhibitor_pick_text(text, text, text);
DROP FUNCTION IF EXISTS public._company_merge_exhibitor_field_strategy(jsonb, uuid, text);
DROP FUNCTION IF EXISTS public._company_merge_exhibitor_texts_conflict(text, text);

CREATE OR REPLACE FUNCTION public._company_merge_exhibitor_strategy(
  p_resolutions jsonb,
...
BEGIN
  FOR v_item IN
    SELECT value
    FROM jsonb_array_elements(COALESCE(p_resolutions -> 'exhibitor_conflicts', '[]'::jsonb))
  LOOP
...
REVOKE ALL ON FUNCTION public._company_merge_exhibitor_strategy(jsonb, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public._company_merge_exhibitor_strategy(jsonb, uuid) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public._company_merge_exhibitor_strategy(jsonb, uuid) TO service_role;
...
AS $$
  ...
  OR EXISTS (
    SELECT 1
    FROM public.event_exhibitors ex
    WHERE ex.company_id = p_duplicate_company_id
  )
  OR EXISTS (
    SELECT 1
  ...
  OR (
    to_regclass('public.event_partner_alumni_version_companies') IS NOT NULL
    AND EXISTS (
      SELECT 1
...
BEGIN
  ...
  IF COALESCE(v_normalized -> 'organizer_conflicts', 'null'::jsonb) = 'null'::jsonb THEN
    v_normalized := jsonb_set(v_normalized, '{organizer_conflicts}', '[]'::jsonb, true);
  END IF;

  IF COALESCE(v_normalized -> 'exhibitor_conflicts', 'null'::jsonb) = 'null'::jsonb THEN
    v_normalized := jsonb_set(v_normalized, '{exhibitor_conflicts}', '[]'::jsonb, true);
  END IF;
  ...
  v_required_organizer := COALESCE(
    p_preview -> 'required_resolutions' -> 'organizer_conflicts',
    '[]'::jsonb
  );

  FOR v_item IN SELECT value FROM jsonb_array_elements(v_required_organizer)
  LOOP
    v_edition_id := (v_item #>> '{}')::uuid;
    PERFORM public._company_merge_organizer_strategy(v_normalized, v_edition_id);
  END LOOP;

  v_required_exhibitor := COALESCE(
    p_preview -> 'required_resolutions' -> 'exhibitor_conflicts',
    '[]'::jsonb
  );

  FOR v_item IN SELECT value FROM jsonb_array_elements(v_required_exhibitor)
  LOOP
    v_edition_id := (v_item #>> '{}')::uuid;
    PERFORM public._company_merge_exhibitor_strategy(v_normalized, v_edition_id);
  END LOOP;
...
REVOKE EXECUTE ON FUNCTION public._company_merge_validate_resolutions(jsonb, jsonb) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public._company_merge_validate_resolutions(jsonb, jsonb) TO service_role;

CREATE OR REPLACE FUNCTION public._company_merge_process_exhibitors(
...
DECLARE
  ...
  v_edition_id uuid;
  v_strategy text;
  v_dup public.event_exhibitors%ROWTYPE;
  v_can public.event_exhibitors%ROWTYPE;
  v_repointed integer := 0;
  v_deleted integer := 0;
  v_updated integer := 0;
  v_repointed_log jsonb := '[]'::jsonb;
  v_deleted_log jsonb := '[]'::jsonb;
  v_updated_log jsonb := '[]'::jsonb;
  v_affected_editions uuid[] := ARRAY[]::uuid[];
  v_repointed_edition_ids uuid[] := ARRAY[]::uuid[];
  v_now timestamp without time zone := now();
BEGIN
  FOR v_conflict IN
    SELECT value
    FROM jsonb_array_elements(COALESCE(p_preview -> 'exhibitor_conflicts', '[]'::jsonb))
  LOOP
    v_edition_id := (v_conflict ->> 'event_edition_id')::uuid;
    v_strategy := public._company_merge_exhibitor_strategy(p_resolutions, v_edition_id);

    SELECT * INTO v_dup
    FROM public.event_exhibitors
    WHERE id = (v_conflict -> 'duplicate_link' ->> 'event_exhibitor_id')::uuid;

    SELECT * INTO v_can
    FROM public.event_exhibitors
    WHERE id = (v_conflict -> 'canonical_link' ->> 'event_exhibitor_id')::uuid;

    IF v_strategy = 'keep_canonical' THEN
      v_deleted_log := v_deleted_log || jsonb_build_array(
        jsonb_build_object(
          'id', v_dup.id,
          'row', jsonb_build_object(
            'company_id', v_dup.company_id,
            'event_editions_id', v_dup.event_editions_id,
            'tier_rank', v_dup.tier_rank,
            'tier_label', v_dup.tier_label,
            'display_order', v_dup.display_order
          ),
          'reason', 'exhibitor_conflict_keep_canonical',
          'event_edition_id', v_edition_id
        )
      );
      DELETE FROM public.event_exhibitors WHERE id = v_dup.id;
      v_deleted := v_deleted + 1;
      v_affected_editions := array_append(v_affected_editions, v_edition_id);
    ELSIF v_strategy = 'keep_duplicate_tier' THEN
      v_updated_log := v_updated_log || jsonb_build_array(
        jsonb_build_object(
          'id', v_can.id,
          'before', jsonb_build_object(
            'tier_rank', v_can.tier_rank,
            'tier_label', v_can.tier_label,
            'display_order', v_can.display_order
          ),
          ...
          'reason', 'exhibitor_conflict_keep_duplicate_tier'
        )
      );
  ...
  WITH repointed AS (
    UPDATE public.event_exhibitors ex
    SET
      company_id = p_canonical_company_id,
      updated_at = v_now
    WHERE ex.company_id = p_duplicate_company_id
    RETURNING ex.id, ex.event_editions_id, ex.tier_rank, ex.tier_label, ex.display_order
  )
  SELECT
    COUNT(*)::integer,
    COALESCE(
    ...
    COALESCE(array_agg(DISTINCT r.event_editions_id), ARRAY[]::uuid[])
  INTO v_repointed, v_repointed_log, v_repointed_edition_ids
  FROM repointed r;

  v_affected_editions := (
    SELECT COALESCE(array_agg(DISTINCT edition_id), ARRAY[]::uuid[])
    FROM unnest(v_affected_editions || v_repointed_edition_ids) AS edition_id
  ...
  RETURN jsonb_build_object(
    ...
    'deleted', v_deleted,
    'updated', v_updated,
    'repointed_log', v_repointed_log,
    'deleted_log', v_deleted_log,
    'updated_log', v_updated_log,
    'affected_edition_ids', COALESCE(
      (SELECT jsonb_agg(DISTINCT edition_id ORDER BY edition_id)
       FROM unnest(v_affected_editions) AS edition_id
       WHERE edition_id IS NOT NULL),
      '[]'::jsonb
    )
  );
...
REVOKE ALL ON FUNCTION public._company_merge_process_exhibitors(uuid, uuid, jsonb, jsonb) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public._company_merge_process_exhibitors(uuid, uuid, jsonb, jsonb) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public._company_merge_process_exhibitors(uuid, uuid, jsonb, jsonb) TO service_role;
...
DECLARE
  ...
  v_event_exhibitors_to_repoint integer;
...
BEGIN
  ...
  SELECT COUNT(*)::integer
  INTO v_event_exhibitors_to_repoint
  FROM public.event_exhibitors ee_x
  WHERE ee_x.company_id = p_duplicate_company_id;
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'event_edition_id', ex_dup.event_editions_id,
        'edition_name', ee.name,
        'edition_year', ee.year,
        'edition_slug', ee.slug,
        'canonical_link', jsonb_build_object(
          'event_exhibitor_id', ex_can.id,
          'tier_rank', ex_can.tier_rank,
          'tier_label', ex_can.tier_label,
          'display_order', ex_can.display_order
        ),
        'duplicate_link', jsonb_build_object(
          'event_exhibitor_id', ex_dup.id,
          'tier_rank', ex_dup.tier_rank,
          'tier_label', ex_dup.tier_label,
          'display_order', ex_dup.display_order
        )
      )
      ORDER BY ee.year DESC NULLS LAST, ee.name
    ),
  ...
  INTO v_exhibitor_conflicts
  FROM public.event_exhibitors ex_dup
  INNER JOIN public.event_exhibitors ex_can
    ON ex_can.event_editions_id = ex_dup.event_editions_id
    AND ex_can.company_id = p_canonical_company_id
  INNER JOIN public.event_editions ee
    ON ee.id = ex_dup.event_editions_id
  WHERE ex_dup.company_id = p_duplicate_company_id;
  ...
  SELECT COALESCE(jsonb_agg(value -> 'event_edition_id' ORDER BY value -> 'event_edition_id'), '[]'::jsonb)
  INTO v_required_exhibitor
  FROM jsonb_array_elements(v_exhibitor_conflicts);
  ...
  v_executable :=
    ...
    AND jsonb_array_length(v_required_exhibitor) = 0
  ...
  RETURN jsonb_build_object(
    ...
    'impact', jsonb_build_object(
      ...
      'event_exhibitors_to_repoint', v_event_exhibitors_to_repoint,
    ...
    'exhibitor_conflicts', v_exhibitor_conflicts,
    ...
    'required_resolutions', jsonb_build_object(
      ...
      'exhibitor_conflicts', v_required_exhibitor,
...
BEGIN
  ...
  v_execution := jsonb_build_object(
    ...
    'actions', jsonb_build_object(
      ...
      'event_exhibitors_repointed', COALESCE((v_exhibitor_result ->> 'repointed')::integer, 0),
      'event_exhibitors_deleted', COALESCE((v_exhibitor_result ->> 'deleted')::integer, 0),
      'event_exhibitors_updated', COALESCE((v_exhibitor_result ->> 'updated')::integer, 0),
    ...
    'repoint_map', jsonb_build_object(
      ...
      'event_exhibitors', COALESCE(v_exhibitor_result -> 'repointed_log', '[]'::jsonb),
      'event_exhibitors_deleted', COALESCE(v_exhibitor_result -> 'deleted_log', '[]'::jsonb),
      'event_exhibitors_updated', COALESCE(v_exhibitor_result -> 'updated_log', '[]'::jsonb),
      'event_exhibitors_affected_edition_ids', COALESCE(v_exhibitor_result -> 'affected_edition_ids', '[]'::jsonb),
...
COMMENT ON FUNCTION public._company_merge_build_preview(uuid, uuid) IS
  'Read-only company merge preview snapshot (includes exhibitor conflicts).';
...
SELECT public.__restrict_rpc_execute_to_service_role(
  'public._company_merge_process_exhibitors(uuid, uuid, jsonb, jsonb)'::regprocedure
);
SELECT public.__restrict_rpc_execute_to_service_role(
  'public._company_merge_exhibitor_strategy(jsonb, uuid)'::regprocedure
);
